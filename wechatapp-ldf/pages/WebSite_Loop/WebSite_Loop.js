...
function getWebSite(that) {
  ...
  var url =
    getApp().globalData.mainUrl + "WX_MiniApps_WebSite_GetWebSiteList.ashx?1=1";
  if (that.data.provinceIndex != 0) {
    url =
      url +
      "&TerritoryCode2=" +
      that.data.province_[that.data.provinceIndex].code;
  }
  ...
  if (that.data.districtIndex != 0) {
    url =
      url +
      "&TerritoryCode4=" +
      that.data.district_[that.data.districtIndex].code;
  }
  if (that.data.market) {
